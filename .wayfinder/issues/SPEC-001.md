...
35. As a time tracker, I want the plugin to detect multiple logical FreshBooks timers after grouping their segments rather than silently choose one, so that exceptional remote state is recoverable.
...
- Define Active Timer as a logical timer identity composed of one or more unlogged Timer Segments. Group records by timer identity before handling zero, one, or multiple logical timers; never count segments as separate timers.
- Do not ship local-only pause, resume, or Duration Correction. Before implementation, characterize the authenticated FreshBooks behavior for pause/resume, running and paused duration replacement, repeated operations, cross-client visibility, stale writes, logging/rounding, billability defaults, and midnight crossing. Record sanitized request shapes, responses, status codes, and resulting web state using disposable entries; never retain credentials, cookies, live identifiers, or full browser captures.
...
- Add verified CLI operations for genuine timer start, pause, resume, and timer-specific note/Duration Correction. A genuine start uses the observed two-step Time Entry flow: create an unlogged entry with `timer: {}`, then apply the authoritative timer identity and selected project/service metadata. A bare `is_logged=false` entry without a timer request is not an Active Timer. Pause closes the open Timer Segment by PUTting its elapsed whole-second duration; it does not send an `is_running` flag. Resume POSTs a new open segment with the existing timer identity and carried project/service metadata. Each operation returns the grouped, confirmed Active Timer; a running correction only rebases local ticking after FreshBooks confirms the aggregate duration.
- Add a high-level CLI Timer Switch operation. It revalidates that the selected project is active, incomplete, permits time tracking, and still contains the selected service before PUTting the timer resource with its Time Entry segments. It confirms logging the selected running or paused timer before starting the next Project Shortcut. Log failure must prevent start. Log success followed by start failure must return an explicit partial result containing the logged Time Entry.
...
- Commit a Duration Correction on Enter or focus loss, show a saving state, and adopt only the FreshBooks-confirmed response. For a running multi-segment timer, preserve fixed completed segments and rebase the open segment so the aggregate matches the requested duration. On rejection, restore the confirmed value and retain an actionable error.
- The Timer tab supports start, pause/resume, explicit log, project/client display, notes, and editable duration. The live display uses `HH:MM:SS`; duration input accepts `MM:SS` or `HH:MM:SS` and carries overflowing minutes and seconds into the normalized display. While the duration field has focus, ticks and refreshes must preserve its text and selection without pausing the underlying timer. Existing two-part drafts retain their original hours-and-minutes meaning through migration to explicit `HH:MM:SS`. Pausing does not implicitly log. Successful completion is subtle; errors persist until resolved or dismissed.
...
- Multiple logical timer identities after segment grouping trigger a recovery state instead of automatic selection. Segments sharing one timer identity never create ambiguity. The user chooses the logical timer to manage; no mutation proceeds against an ambiguous timer set.
...
- The repository now contains the validated root-manifest plugin scaffold and its initial singleton model seam; timer mutations and user-facing workflows remain under implementation.
- FreshBooks models a logical timer as one or more unlogged Timer Segments sharing one timer identity. Pause/resume can create multiple segments, so segment count is not timer count.
- Live characterization and a cleaned-up controlled CLI lifecycle confirmed the web UI's two-step timer start, duration-based pause, segment-appending resume, aggregate correction by rebasing the open segment, timer-resource logging with an authoritative project preflight, and logical timer identity continuity. Stale-write conflict UX and real wall-clock midnight behavior remain release gates.
