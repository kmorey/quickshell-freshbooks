...
- Live characterization confirmed the web UI's two-step timer start, duration-based pause, segment aggregation, correction anchoring, and logging consolidation. The resume request shape, stale-write handling, and real wall-clock midnight behavior still require safe characterization before those CLI behaviors can ship.
