...
- Add verified CLI operations for genuine timer start, pause, resume, and timer-specific note/Duration Correction. A genuine start uses the observed two-step Time Entry flow: create an unlogged entry with `timer: {}`, then apply the authoritative timer identity and selected project/service metadata. A bare `is_logged=false` entry without a timer request is not an Active Timer. Each operation returns the grouped, confirmed Active Timer; a running correction only rebases local ticking after FreshBooks confirms the aggregate duration.
...
- Live characterization confirmed the web UI's two-step timer start, segment aggregation, correction anchoring, and logging consolidation. Pause/resume request shapes, stale-write handling, and real wall-clock midnight behavior still require safe characterization before those CLI behaviors can ship.
