...
- Live characterization confirmed the web UI's two-step timer start, duration-based pause, segment-appending resume, aggregate correction by rebasing the open segment, and logging consolidation. Exact log request changes, stale-write handling, and real wall-clock midnight behavior still require safe characterization before those CLI behaviors can ship.
