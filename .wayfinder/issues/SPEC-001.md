...
- The Timer tab supports start, pause/resume, explicit log, project/client display, notes, and editable duration. The live display uses `HH:MM:SS`; duration input accepts `HH:MM` with optional seconds. Pausing does not implicitly log. Successful completion is subtle; errors persist until resolved or dismissed.
